...
st.title("🌿 Hệ Thống Giám Sát Nhà Kính (Bản Full Cảnh Báo Màu)")

# --- HÀM GỬI EMAIL ---
def send_email_alert(sender_mail, app_password, receiver_mail, vpd, status, temp, humi):
    if vpd > 1.5:
        ly_do, tinh_trang = "Nhiệt độ cao, độ ẩm thấp (Khô nóng).", "Cây mất nước, cháy lá."
        cach_xu_ly = "Bật phun sương, che nắng ngay."
    elif vpd < 0.4:
        ly_do, tinh_trang = "Độ ẩm quá cao (Lạnh ẩm).", "Nguy cơ nấm bệnh, thối rễ."
        cach_xu_ly = "Bật quạt hút, ngừng tưới."
    else:
        ly_do, tinh_trang, cach_xu_ly = "Lệch nhẹ.", "Cây stress.", "Kiểm tra thiết bị."
...
# --- XỬ LÝ DỮ LIỆU (ƯU TIÊN tempKK, humiKK) ---
...
if uploaded_file:
    ...
    if not df.empty:
        ...
        filter_mode = st.sidebar.radio("Thời gian:", ["Tất cả", "Tháng", "Khoảng ngày"])
        ...
        if not df_valid.empty:
            ...
            # BIỂU ĐỒ
            st.plotly_chart(make_subplots(rows=2, cols=1, shared_xaxes=True).add_trace(go.Scatter(x=df_valid['Thời gian'], y=df_valid['VPD'], name="VPD"), 1, 1).add_trace(go.Scatter(x=df_valid['Thời gian'], y=df_valid['temp'], name="Temp"), 2, 1).add_trace(go.Scatter(x=df_valid['Thời gian'], y=df_valid['humi'], name="Humi"), 2, 1), use_container_width=True)

            # --- PHẦN TÔ MÀU BẢNG DỮ LIỆU GẮT ---
            st.subheader("📋 Chi tiết bản ghi (Vùng đỏ là nguy hiểm)")
            
            def style_vpd_rows(row):
                val = row['VPD']
                # Nếu VPD quá cao (màu đỏ đậm)
                if val > 1.5:
                    return ['background-color: #8B0000; color: white; font-weight: bold'] * len(row)
                # Nếu VPD quá thấp (màu cam/đỏ nhạt)
                elif val < 0.4:
                    return ['background-color: #FF4B4B; color: white'] * len(row)
                # Nếu lý tưởng (chữ xanh)
                elif 0.8 <= val <= 1.2:
                    return ['color: #00C851; font-weight: bold'] * len(row)
                return [''] * len(row)

            st.dataframe(
                df_valid[['Thời gian', 'STT', 'temp', 'humi', 'VPD']]
                .sort_values('Thời gian', ascending=False)
                .style.apply(style_vpd_rows, axis=1), # Tô màu nguyên dòng cho rực rỡ
